...
function Itinerary() {
  ...
  const title = tx(it.title_ar, it.title_en);
    ...
    <div className="bg-cream">
      <section className="relative h-[60vh] min-h-[420px] bg-midnight">
        ...
        <div className="relative z-10 mx-auto max-w-7xl h-full flex flex-col justify-end px-6 pb-16 text-cream">
          ...
          <div className="text-[11px] uppercase tracking-[0.4em] text-gold mb-3">{it.duration_days} {tx("يوم", "days")}</div>
          <h1 className="font-display text-5xl md:text-7xl">{title}</h1>
          <p className="mt-6 max-w-2xl text-cream/70">{tx(it.summary_ar, it.summary_en)}</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-3xl px-6 space-y-12">
          {days.map((d: any) => (
            <article key={d.day} className="border-t border-sand pt-8">
              <div className="text-[11px] uppercase tracking-[0.4em] text-gold mb-2">{tx(`اليوم ${d.day}`, `Day ${d.day}`)}</div>
              <h2 className="font-display text-3xl text-midnight">{tx(d.title_ar, d.title_en)}</h2>
              <p className="mt-4 leading-loose text-charcoal/80">{tx(d.content_ar, d.content_en)}</p>
            </article>
          ))}
        </div>
      </section>
